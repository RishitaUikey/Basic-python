...
const selectedQuestions = shuffledQuestions.slice(0, 25);
    ...
    async function stopEmotionDetection() {
        // Make an AJAX request to stop emotion detection
        await $.get("/stop_emotion_detection", function (data) {
...
function openVideoWindow() {
    var videoFolder = "{{ url_for('static', filename='videos/') }}";
    var videoName = "guided.mp4";
    var videoUrl = videoFolder + videoName;

    var videoWindow = window.open(videoUrl, "_blank", "width=800, height=600");

    if (!videoWindow) {
        alert("Error: Unable to open video. Please check your browser settings.");
    }
}
